...
void plot_ranks(const std::span<StockRanks*> all_stock_ranks) {
    ...
    for (const auto& stock_rank : all_stock_ranks) {
        if (stock_rank == nullptr || stock_rank->data_size == 0) continue;
        ...
        for (size_t i = 0; i < stock_rank->data_size; ++i) {
            const Date* d = stock_rank->dates[i];
            int date_int = d->year * 10000 + d->month * 100 + d->day;
            rank_map[date_int] = static_cast<double>(stock_rank->rank_per_day[i]);
        }
        std::vector<double> y;
        ...
        plt::plot(x_numeric, y, {{"label", stock_rank->stock_symbol}});
    }
    std::vector<double> x_positions(all_dates.size());
    for (size_t i = 0; i < all_dates.size(); i+= 23) {
...
extern "C" {
    ...
    void convert_and_plot(const AllStockRanks* all) {
        const std::span stocks(all->symbol_to_rank_map->stock_to_rank, RANK_MAP_SIZE);
        plot_ranks(stocks);
    }
